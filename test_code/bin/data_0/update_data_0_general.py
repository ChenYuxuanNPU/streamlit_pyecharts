...
def update():
    ...
    # 下面在字典中更新数据库查询结果
    # 先统计下总人数
    sql_sentence = gd.generate_sql_sentence(kind="在编", info_num=-1, info=[""], scope="全区")
    ...
    json_data['在编']['全区']['所有学段']['总人数'] = copy.deepcopy(result)
    result = []

    ###
    # 在编人员最高学历统计
    ###
    sql_sentence = gd.generate_sql_sentence(kind="在编", info_num=1, info=["最高学历"], scope="全区")
    ...
    json_data['在编']['全区']['所有学段']['最高学历'] = copy.deepcopy(result)
    result = []

    ###
    # 在编人员最高职称统计
    ###
    sql_sentence = gd.generate_sql_sentence(kind="在编", info_num=1, info=["最高职称"], scope="全区")
    ...
    json_data['在编']['全区']['所有学段']['最高职称'] = copy.deepcopy(result)
    result = []
    ...
    # 在编人员骨干教师统计
    ###
    sql_sentence = gd.generate_sql_sentence(kind="在编", info_num=1, info=["骨干教师"], scope="全区")
    ...
    json_data['在编']['全区']['所有学段']['骨干教师'] = copy.deepcopy(result)
    result = []
    ...
    ###
    # 这里统计有多少是主持人
    sql_sentence = gd.generate_sql_sentence(kind="在编", info_num=-1, info=["三名工作室"],
    ...
    json_data['在编']['全区']['所有学段']['三名工作室']['三名工作室主持人'] = copy.deepcopy(result)
    result = []
    ...
    json_data['在编']['全区']['所有学段']['三名工作室']['无'] = copy.deepcopy(result)
    result = []
