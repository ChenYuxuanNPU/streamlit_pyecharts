...
import sqlite3
from test_code.bin.update_data_0 import make_sql_sentence


def create_table(database_name,table_name):

    # 用来连接数据库插入数据
    #C:\Users\1012986131\Desktop\python\streamlit_pyecharts\update_database\data_source\database\data_0.db
    conn = sqlite3.connect("C:\\Users\\1012986131\\Desktop\\python\\streamlit_pyecharts\\database\\" + database_name)
    c = conn.cursor()

    sql_sentence = make_sql_sentence.make_sql_sentence()
    ...
    try:
        ...
        print("执行mysql语句时报错：%s （update_data_0.make_database.py）" % e)
