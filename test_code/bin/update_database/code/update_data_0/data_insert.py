...
import sqlite3
from test_code.bin.update_data_0 import make_input_data
...
def insert_data(database_name,table_name):
    ...
    print(f"import来的数据长度为：{str(len(result[0]))}（update_data_0.data_insert.py）")
    ...
    try:
        ...
        print("执行mysql语句时报错：%s （update_data_0.data_insert.py）" % e)

    finally:
        conn.commit()
        conn.close()
        print("数据插入成功（update_data_0.data_insert.py）")
