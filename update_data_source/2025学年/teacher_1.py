...
lst = {
    ...
    "list_av": ["直管", "永平", "石井", "新市", "江高", "人和", "太和", "钟落潭"],
    "list_aw": ["高中", "初中", "小学", "中职", "幼儿园", "其他"],
    "list_ax": ["是", "否"],
    "list_ay": ["公办临聘教师", "民办学校教师", "政府雇员"],
    "list_az": ["博士学位", "硕士学位", "学士学位", "无"],
    "list_ba": ["其他", "无"] + [item[0] for item in read_xlsx_to_list(
        r"C:\Users\1012986131\Desktop\python\streamlit_pyecharts\参考资料\2025年6月院校名单.xlsx")[1:]],
}
...
def output_excel_0(title: list, data: list, file_name: str, area_name: str):
    ...
    def add_excel_func():
        ...
        def check_blank():
            ...
            for i in range(1, 3):  # BS是Excel中的第702列
                # 生成列名（BA-BZ）
                column_name = chr(ord(column_letter) + i - 1)
        ...
        # 首先判断前三列学校信息会不会有不同的
        # 列表代表了对应的列标
        for i, cell in enumerate(["A", "B", "C", "AV"]):
        ...
        ws_check[f'K37'] = "AZ/BA/BB列非学历教育最高学位-学士学位及以上填写校名"
        ws_check[
            f'K38'] = '=IF(SUMPRODUCT(ISNUMBER(MATCH(数据表!AZ:AZ,{"学士学位","硕士学位","博士学位"},0))*ISNUMBER(MATCH(数据表!BA:BA,{"其他","无"},0))*ISNUMBER(MATCH(数据表!BB:BB,{"其他","无"},0)))>0,0,1)'
        ws_check[f'K38'].font = font
        cell_list.append(f'K38')

        ws_check[f'K40'] = "BB列非学历教育毕业院校应选未选"
        ws_check[
            f'K41'] = '=IF(SUMPRODUCT(--(NOT(ISERROR(MATCH(数据表!BB:BB,下拉列表信息!BA:BA,0)))))-SUM(COUNTIF(数据表!BB:BB,{"无","其他"}))>0,0,1)'
    ...
    write_list_to_txt(
        route=fr"C:\Users\1012986131\Desktop\python\streamlit_pyecharts\update_data_source\2025学年\output\编外\{area}\{file_name}\下拉列表内容.txt")

    write_name_list_to_txt(
        route=fr"C:\Users\1012986131\Desktop\python\streamlit_pyecharts\update_data_source\2025学年\output\编外\{area}\{file_name}\已有人员名单.txt",
        name_list=name_list)

    wb.save(
        fr"C:\Users\1012986131\Desktop\python\streamlit_pyecharts\update_data_source\2025学年\output\编外\{area}\{file_name}\{file_name}.xlsx")


if __name__ == '__main__':
    data = read_xlsx_to_list(
        file_path=fr"C:\Users\1012986131\Desktop\python\streamlit_pyecharts\update_data_source\2025学年\data\teacher_1.xlsx")[
        1:]

    output = {item: {} for item in ["直管", "新市", "永平", "石井", "江高", "人和", "太和", "钟落潭"]}

    for item in data:
        if item[0] not in output[item[-7]].keys():
            output[item[-7]][item[0]] = []

        output[item[-7]][item[0]].append(item)
