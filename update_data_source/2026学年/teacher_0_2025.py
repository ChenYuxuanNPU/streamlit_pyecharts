...
title_0 = [
    ...
    "校内其他职务", "非学历教育最高学位（如同等学力申硕）", "非学历教育最高学位毕业院校",
    "非学历教育最高学位毕业院校（境外高校或其他院校）", "非学历教育毕业时间", "备注"
]
lst = {
    ...
    "list_bq": ["博士学位", "硕士学位", "学士学位", "无"],
    "list_br": ["其他", "无"] + [item[0] for item in read_xlsx_to_list(
...
def output_excel_0(title: list, data: list, file_name: str, area_name: str):
    ...
    def add_excel_func():
        ...
        # 填写完成的判断
        ws_check['A1'] = '检查无误'
        ws_check['A2'] = f'=IF(AND({"=1,".join(["A5"] + cell_list)}=1),1,0)'
    ...
    write_list_to_txt(
        route=fr"C:\Users\1012986131\Desktop\python\streamlit_pyecharts\update_data_source\2026学年\output\在编\{area}\{file_name}\下拉列表内容.txt")

    write_name_list_to_txt(
        route=fr"C:\Users\1012986131\Desktop\python\streamlit_pyecharts\update_data_source\2026学年\output\在编\{area}\{file_name}\已有人员名单.txt",
        name_list=name_list)

    wb.save(
        fr"C:\Users\1012986131\Desktop\python\streamlit_pyecharts\update_data_source\2026学年\output\在编\{area}\{file_name}\{file_name}.xlsx")
...
if __name__ == '__main__':
    ...
    for item in data:
        ...
        if item[0] not in output[item[-12]].keys():
            output[item[-12]][item[0]] = []

        output[item[-12]][item[0]].append(item)
