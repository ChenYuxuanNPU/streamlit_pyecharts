...
title_0 = ["校名", "学校类型", "统一社会信用代码", "姓名", "身份证号", "性别", "民族",
           "籍贯", "婚姻状况", "政治面貌", "入党（团）时间", "参加工作前学历", "参加工作前学位",
           "参加工作前毕业院校（普通高校）", "参加工作前毕业院校（境外高校或其他院校）", "参加工作前毕业时间",
           "参加工作前所学专业", "最高学历", "最高学历对应学位", "最高学历毕业院校（普通高校）",
           "最高学历毕业院校（境外高校或其他院校）", "最高学历毕业时间", "最高学历所学专业",
           "参加工作时间", "进入白云教育系统时间", "入现单位时间", "行政职务",
           "行政职务任职时间", "任教年级", "主教学科", "现持有最高职称", "职称证专业",
           "职称认定时间", "普通话水平", "教师资格学段", "教师资格学科", "骨干教师",
           "四名工作室主持人", "四名工作室主持人确定时间", "市级以上荣誉", "市级以上人才项目",
           "常住地址", "手机号", "紧急联系人", "与紧急联系人的关系", "紧急联系方式",
           "区域", "任教学段", "是否音体美专任教师", "教师身份", "非学历教育最高学位",
           "非学历教育最高学位毕业院校（普通高校）", "非学历教育最高学位毕业院校（境外高校或其他院校）",
           "非学历教育毕业时间", "备注", ]
...
lst = {
    ...
    "list_n": ["其他", "无"] + [item[0] for item in read_xlsx_to_list(
        r"C:\Users\1012986131\Desktop\python\streamlit_pyecharts\参考资料\2026年6月院校名单.xlsx")[1:]],
    "list_r": ["博士研究生", "硕士研究生", "本科", "专科", "中专（非师范）", "中师", "高中", "初中"],
    "list_s": ["博士学位", "硕士学位", "学士学位", "无"],
    "list_t": ["其他", "无"] + [item[0] for item in read_xlsx_to_list(
        r"C:\Users\1012986131\Desktop\python\streamlit_pyecharts\参考资料\2026年6月院校名单.xlsx")[1:]],
    "list_aa": ["书记（正校级）", "副书记（副校级）", "正校级", "副校级", "中层正职", "中层副职", "少先队大队辅导员",
                "少先队副大队辅导员", "工会主席", "工会副主席", "团委书记", "团委副书记", "无"],
    "list_ac": generate_subsets(
    ...
    "list_ae": ["正高级教师", "高级教师", "一级教师", "二级教师", "三级教师", "高级职称（非中小学系列）",
                "中级职称（非中小学系列）", "初级职称（非中小学系列）", "未取得职称"],
    "list_ah": ["一级甲等", "一级乙等", "二级甲等", "二级乙等", "三级甲等", "三级乙等", "无"],
    "list_ai": ["幼儿园", "小学", "初级中学", "高级中学", "中职专业课", "中职实习指导教师", "高等学校", "无"],
    "list_ak": ["广东省骨干教师", "广州市骨干教师", "白云区骨干教师", "其他", "无"],
    "list_au": ["直管", "永平", "石井", "新市", "江高", "人和", "太和", "钟落潭"],
    "list_av": ["高中", "初中", "小学", "中职", "幼儿园", "其他"],
    "list_aw": ["是", "否"],
    "list_ax": ["公办临聘教师", "民办学校教师", "政府雇员"],
    "list_ay": ["博士学位", "硕士学位", "学士学位", "无"],
    "list_az": ["其他", "无"] + [item[0] for item in read_xlsx_to_list(
        r"C:\Users\1012986131\Desktop\python\streamlit_pyecharts\参考资料\2026年6月院校名单.xlsx")[1:]],
}
...
def ensure_folders_exist_or_clear(kind: str, area_name1="", school_name1=""):
    ...
    if school_name1 == "":
        base_dir = fr"C:\Users\1012986131\Desktop\python\streamlit_pyecharts\update_data_source\2026学年\output\{kind}"
        ...
        base_dir = fr"C:\Users\1012986131\Desktop\python\streamlit_pyecharts\update_data_source\2026学年\output\{kind}\{area_name1}"
...
def output_excel_0(title: list, data: list, file_name: str, area_name: str):
    ...
    def add_excel_func():
        ...
        def check_blank():
            ...
            for i in range(1, 4):  # BS是Excel中的第702列
                # 生成列名（BA-BZ）
                column_name = chr(ord(column_letter) + i - 1)
        ...
        # 首先判断前三列学校信息会不会有不同的
        # 列表代表了对应的列标
        for i, cell in enumerate(["A", "B", "C", "AU"]):
        ...
        # 手机号位数
        for i, cell in enumerate(["AQ", "AT"], start=4):
        ...
        ws_check[f'K37'] = "AY/AZ/BA列非学历教育最高学位-学士学位及以上填写校名"
        ws_check[
            f'K38'] = '=IF(SUMPRODUCT(ISNUMBER(MATCH(数据表!AY:AY,{"学士学位","硕士学位","博士学位"},0))*ISNUMBER(MATCH(数据表!AZ:AZ,{"其他","无"},0))*ISNUMBER(MATCH(数据表!BA:BA,{"其他","无"},0)))>0,0,1)'
        ws_check[f'K38'].font = font
        cell_list.append(f'K38')

        ws_check[f'K40'] = "BA列非学历教育毕业院校应选未选"
        ws_check[
            f'K41'] = '=IF(SUMPRODUCT(--(NOT(ISERROR(MATCH(数据表!BA:BA,下拉列表信息!AZ:AZ,0)))))-SUM(COUNTIF(数据表!BA:BA,{"无","其他"}))>0,0,1)'
        ws_check[f'K41'].font = font
        cell_list.append(f'K41')

        #  todo:要加上填了本科及以上学历学位的时候必须填毕业时间
        ...
        # # 这里判断具体空的列数
        # for i, cell in enumerate(generate_letter_sequence("A", "BB")):
        #     ws_check[f'M{3 * i + 1}'] = f'{cell}列空单元格'
        #     ws_check[f'M{3 * i + 2}'] = f"=IF(COUNTBLANK(数据表!{cell}:{cell})=COUNTA(数据表!{cell}:{cell}),1,0)"
    ...
    write_list_to_txt(
        route=fr"C:\Users\1012986131\Desktop\python\streamlit_pyecharts\update_data_source\2026学年\output\编外\{area}\{file_name}\下拉列表内容.txt")

    write_name_list_to_txt(
        route=fr"C:\Users\1012986131\Desktop\python\streamlit_pyecharts\update_data_source\2026学年\output\编外\{area}\{file_name}\已有人员名单.txt",
        name_list=name_list)

    wb.save(
        fr"C:\Users\1012986131\Desktop\python\streamlit_pyecharts\update_data_source\2026学年\output\编外\{area}\{file_name}\{file_name}.xlsx")


if __name__ == '__main__':
    data = read_xlsx_to_list(
        file_path=fr"C:\Users\1012986131\Desktop\python\streamlit_pyecharts\update_data_source\2026学年\data\teacher_1_2025.xlsx")[
        1:]
    ...
    for item in data:
        if item[0] not in output[item[-9]].keys():
            output[item[-9]][item[0]] = []

        output[item[-9]][item[0]].append(item)
