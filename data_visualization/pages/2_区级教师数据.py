...
def show_period(period: str, data: dict,) -> None:
    ...
    with st.container(border=False):
        ...
        with c1:
            visual_func.draw_pie(data=data[year]["在编"]["全区"][period]["年龄"], title="年龄")
...
if year is not None:
    ...
    with st.container(border=True):
        ...
        if "所有学段" in period_list:
            st.success(f"在编教职工总人数：{json_data[year]['在编']['全区']['所有学段']['总人数']}")
            ...
            with st.container(border=False):
                ...
                with col0:
                    # 在编片区统计
                    visual_func.draw_pie(data=json_data[year]["在编"]["全区"]["所有学段"]["片区统计"], title="片区统计")

                    # 在编学历统计
                    visual_func.draw_pie(data=json_data[year]["在编"]["全区"]["所有学段"]["最高学历"], title="最高学历")
                ...
                with col2:
                    # 在编年龄统计
                    visual_func.draw_pie(data=json_data[year]["在编"]["全区"]["所有学段"]["年龄"], title="年龄")

                    # 在编行政职务统计
                    visual_func.draw_pie(data=json_data[year]["在编"]["全区"]["所有学段"]["行政职务"], title="行政职务")
                ...
                with col0:
                    # 在编学科统计
                    visual_func.draw_bar(data=json_data[year]["在编"]["全区"]["所有学段"]["主教学科"], title="主教学科", end=70)

                with col1:
                    # 在编毕业院校统计
                    visual_func.draw_bar(data=json_data[year]["在编"]["全区"]["所有学段"]["院校级别"], title="毕业院校",
                ...
                with col0:
                    # 在编骨干教师统计
                    visual_func.draw_pie(data=json_data[year]["在编"]["全区"]["所有学段"]["骨干教师"], title="骨干教师")
                ...
                # 教师分布统计
                visual_func.draw_bar(data=json_data[year]["在编"]["全区"]["所有学段"]["教师分布"], title="教师分布", end=50)
    ...
    with st.container(border=True):
        ...
        st.info(f"编外教职工总人数：{json_data[year]['编外']['全区']['所有学段']['总人数']}")

        col0, col1, col2 = st.columns(spec=3)

        with col0:
            # 编外片区统计
            visual_func.draw_pie(data=json_data[year]["编外"]["全区"]["所有学段"]["片区统计"], title="片区统计")

            # 编外学段统计
            visual_func.draw_pie(data=json_data[year]["编外"]["全区"]["所有学段"]["学段统计"], title="学段统计")
        ...
        # 教师分布统计
        visual_func.draw_bar(data=json_data[year]["编外"]["全区"]["所有学段"]["教师分布"], title="教师分布", end=100)

        col0, col1, col2 = st.columns(spec=3)

        with col0:
            # 编外教师资格统计
            visual_func.draw_pie(data=json_data[year]["编外"]["全区"]["所有学段"]["教师资格"], title="教师资格")

        with col1:
            # 编外中小学教师资格统计
            visual_func.draw_pie(data=json_data[year]["编外"]["全区"]["中小学"]["教师资格"], title="中小学")

        with col2:
            # 编外幼儿园教师资格统计
            visual_func.draw_pie(data=json_data[year]["编外"]["全区"]["幼儿园"]["教师资格"], title="幼儿园")
