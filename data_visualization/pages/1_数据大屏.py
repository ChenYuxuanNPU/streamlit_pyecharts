    ...
    with st.container(border=True):
        ...
        with col1:
            ...
            visual_func.draw_dataframe(
                data=pd.DataFrame(
                    [
                        ["学校数", "合计", json_data[year]["合计"]["合计学校数"]],
                        ["1.", "公办学校数", json_data[year]["合计"]["公办学校数"]],
                        ["2.", "民办学校数", json_data[year]["合计"]["民办学校数"]],
                        ["学生数", "合计", json_data[year]["合计"]["合计学生数"]],
                        ["公办学校学生数", "小计", json_data[year]["合计"]["公办学校学生数"]],
                        ["1.", "公办学校白云区户籍学生数", json_data[year]["合计"]["公办学校白云区户籍学生数"]],
                        ["2.", "公办学校广州市户籍学生数", json_data[year]["合计"]["公办学校广州市户籍学生数"]],
                        ["3.", "公办学校非白云区户籍学生数", json_data[year]["合计"]["公办学校非白云区户籍学生数"]],
                        ["4.", "公办学校非广州市户籍学生数", json_data[year]["合计"]["公办学校非广州市户籍学生数"]],
                        ["民办学校学生数", "小计", json_data[year]["合计"]["民办学校学生数"]],
                        ["1.", "民办学校白云区户籍学生数", json_data[year]["合计"]["民办学校白云区户籍学生数"]],
                        ["2.", "民办学校广州市户籍学生数", json_data[year]["合计"]["民办学校广州市户籍学生数"]],
                        ["3.", "民办学校非白云区户籍学生数", json_data[year]["合计"]["民办学校非白云区户籍学生数"]],
                        ["4.", "民办学校非广州市户籍学生数", json_data[year]["合计"]["民办学校非广州市户籍学生数"]],
                        ["教职工数", "合计", json_data[year]["合计"]["合计教职工数"]],
                        ["1.", "公办学校教职工数", json_data[year]["合计"]["公办学校教职工数"]],
                        ["2.", "民办学校教职工数", json_data[year]["合计"]["民办学校教职工数"]],
                        ["专任教师数", "合计", json_data[year]["合计"]["合计专任教师数"]],
                        ["1.", "公办学校专任教师数", json_data[year]["合计"]["公办学校专任教师数"]],
                        ["2.", "民办学校专任教师数", json_data[year]["合计"]["民办学校专任教师数"]],
                        ["班额数", "合计", json_data[year]["合计"]["合计班额数"]],
                    ],
                    columns=["项目", "细分项", "人数"]
                )
            )
...
def show_period_detail_info(year: str) -> None:
    ...
    with (st.container(border=True)):
        ...
        if period is not None:
            ...
            if int(json_data[year][period]["合计学校数"]) > 1 and json_data[year][period]["公办学校数"] > 0 and json_data[year][
                ...
                with col0:
                    ...
                    visual_func.draw_pie(
                        data=dict([
                            ...
                            ["市内外区",
                             json_data[year][period]["公办学校广州市户籍学生数"] - json_data[year][period][
                                 "公办学校白云区户籍学生数"]],
                            ["广州市外", json_data[year][period]["公办学校非广州市户籍学生数"]]
                        ]),
                        title="公办学校户籍分布",
                        formatter="{c}--占比{d}%",
                        pos_left="35%"
                ...
                with col1:
                    ...
                    visual_func.draw_dataframe(
                        data=pd.DataFrame(
                            [
                                ["合计学校数", json_data[year][period]["合计学校数"]],
                                ["合计学生数", json_data[year][period]["合计学生数"]],
                                ["公办学校学生数", json_data[year][period]["公办学校学生数"]],
                                ["民办学校学生数", json_data[year][period]["民办学校学生数"]],
                                ["合计教职工数", json_data[year][period]["合计教职工数"]],
                                ["合计专任教师数", json_data[year][period]["合计专任教师数"]],
                                ["合计班额数", json_data[year][period]["合计班额数"]],
                            ],
                            columns=["项目", "人数"]
                        )
                    )
                ...
                with col2:
                    ...
                    visual_func.draw_pie(
                        data=dict([
                            ...
                            ["市内外区",
                             json_data[year][period]["民办学校广州市户籍学生数"] - json_data[year][period][
                                 "民办学校白云区户籍学生数"]],
                            ["广州市外", json_data[year][period]["民办学校非广州市户籍学生数"]]
                        ]),
                        title="民办学校户籍分布",
                        formatter="{c}--占比{d}%",
                        pos_left="35%"
            ...
            # 纯公/民办
            elif json_data[year][period]["公办学校数"] > 0 or json_data[year][period]["民办学校数"] > 0:

                st.warning(
                    f"由于白云区内的{period}均为{'公办' if json_data[year][period]["公办学校数"] > 0 else '民办'}学校，数据将通过表格的形式展示",
                    icon='⚠️')
                ...
                with col_mid:
                    ...
                    visual_func.draw_dataframe(
                        data=pd.DataFrame(
                            [
                                ["办学类型", f"{'公办' if json_data[year][period]["公办学校数"] > 0 else '民办'}"],
                                ["合计学校数", str(json_data[year][period]["合计学校数"])],
                                ["合计班额数", str(json_data[year][period]["合计班额数"])],
                                ["合计学生数", str(json_data[year][period]["合计学生数"])],
                                ["合计教职工数", str(json_data[year][period]["合计教职工数"])],
                                ["合计专任教师数", str(json_data[year][period]["合计专任教师数"])],
                            ],
                            columns=["项目", "信息"]
                        ),
                        hide_index=False,
